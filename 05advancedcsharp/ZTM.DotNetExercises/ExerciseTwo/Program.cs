...
void SecurityIDMenu()
{
    Console.WriteLine("Check to see if your accounts have a security ID.");
    Console.Write("Enter 1 for checking, 2 for premium: ");
    int choice = Convert.ToInt32(Console.ReadLine());
    if (choice == 1)
    {
        Console.WriteLine($"Has security ID: {checking.HasSecurityID()}");
    }
    if (choice == 2)
    {
        Console.WriteLine($"Has security ID: {premium.HasSecurityID()}");
    }
}